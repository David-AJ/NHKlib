...
导入本库使用 from NHKlib2 import *
传入参数的格式设定：
	每一个Labeled Graph数据都使用两个字典Graph,Glabel存储,存储格式

	Graph = array([[]])         Graph为图的连接矩阵,一经输入不再改变
...
def NH(Graph, Glabel, d, shape):
    """NH(Graph, Glabel, d, shape)
    Graph->1-N array []
    Glabel->{nodeid:label}
    d->the length of binary array
    shape->reshape Graph array to shape*shape array

    return a list of sorted with N-Hashed label"""
    Graph = Graph.reshape(shape,shape)
    newGlabel = {}
    for Vi in Glabel.keys():
    	# print "Updating NH Labels..."
        # update node Vi's label
        label = ROT(Glabel[Vi], 1, d)
        for neighbor in np.argwhere(Graph[Vi]==1).reshape(np.argwhere(Graph[Vi]==1).size).astype(int):
            label = label ^ Glabel[neighbor]
        newGlabel[Vi] = label
    # print "Updating NH Labels finished!"
    newGlabel = newGlabel.values()
    sort(newGlabel,d)
    return [g for g in newGlabel]


def CSNH(Graph, Glabel, d, shape):
    """CSNH(Graph, Glabel, d, shape)
    Graph->array([[]])
    Glabel->{nodeid:label}
    d->the length of binary array
    shape->reshape Graph array to shape*shape array

    return a list of sorted with CSN-Hashed label"""
    Graph = Graph.reshape(shape,shape)
    newGlabel = {}
    for Vi in Glabel.keys():
        ...
        label = ROT(Glabel[Vi], 1, d)
        neighbor = [Glabel[i] for i in np.argwhere(Graph[Vi]==1).reshape(np.argwhere(Graph[Vi]==1).size).astype(int)]
